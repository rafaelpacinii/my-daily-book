...
export { loadReadingGoalsListViewModel } from './goals-list-loader';

export function useReadingGoalsScreen(api: ApplicationApi) {
  const [status, setStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');
  const [refreshing, setRefreshing] = useState(false);
  const [viewModel, setViewModel] = useState<ReadingGoalsListViewModel | null>(null);
  const [error, setError] = useState<unknown>(null);
  const loadingRef = useRef(false);
  const loadedRef = useRef(false);
  const viewModelRef = useRef<ReadingGoalsListViewModel | null>(null);
  ...
  const load = useCallback((mode: 'initial' | 'refresh') => {
    ...
    loadReadingGoalsListViewModel(api)
      .then((next) => {
